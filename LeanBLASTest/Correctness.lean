...
/-- Convenience accessor for `Float64Array` values. -/
def get (x : Float64Array) (i : Nat) : Float :=
  x.toFloatArray.get! i

end BLAS.Float64Array
...
theorem axpy_associative (n : Nat) (α β : Float) (x y z : Float64Array)
  ...
  let result1 := daxpy n.toUSize β step1 0 1 z 0 1
...
theorem axpy_distributive (n : Nat) (α β : Float) (x y : Float64Array)
  (hx : x.size ≥ n) (hy : y.size ≥ n) :
  let result1 := daxpy n.toUSize (α + β) x 0 1 y 0 1
  let step1 := daxpy n.toUSize α x 0 1 y 0 1
...
structure BLASCorrectnessProperties (n : Nat) (x y z : Float64Array) : Prop where
  dot_comm : ddot n.toUSize x 0 1 y 0 1 = ddot n.toUSize y 0 1 x 0 1
  norm_pos : (∃ i < n, x.get i ≠ 0) → dnrm2 n.toUSize x 0 1 > 0
  cauchy_schwarz : Float.abs (ddot n.toUSize x 0 1 y 0 1) ≤ dnrm2 n.toUSize x 0 1 * dnrm2 n.toUSize y 0 1
  triangle_ineq : let sum := daxpy n.toUSize 1.0 x 0 1 y 0 1; dnrm2 n.toUSize sum 0 1 ≤ dnrm2 n.toUSize x 0 1 + dnrm2 n.toUSize y 0 1
...
def verify_correctness_properties (n : Nat) (x y z : Float64Array) : IO Bool := do
  ...
  -- Test commutativity
  let dot1 := ddot n.toUSize x 0 1 y 0 1
  let dot2 := ddot n.toUSize y 0 1 x 0 1
  let comm_ok : Bool := decide (Float.abs (dot1 - dot2) < 1e-12)
  IO.println s!"Dot product commutativity: {comm_ok}"
  
  -- Test norm properties
  let norm_x := dnrm2 n.toUSize x 0 1
  let norm_y := dnrm2 n.toUSize y 0 1
  let dot_xy := ddot n.toUSize x 0 1 y 0 1
  let cauchy_ok : Bool := decide (Float.abs dot_xy ≤ norm_x * norm_y + 1e-12)
  IO.println s!"Cauchy-Schwarz inequality: {cauchy_ok}"
  
  -- Test triangle inequality
  let sum := daxpy n.toUSize 1.0 x 0 1 y 0 1
  let norm_sum := dnrm2 n.toUSize sum 0 1
  let triangle_ok : Bool := decide (norm_sum ≤ norm_x + norm_y + 1e-12)
  IO.println s!"Triangle inequality: {triangle_ok}"
  
  -- Test norm squared = dot self
  let dot_self := ddot n.toUSize x 0 1 x 0 1
  let norm_sq_ok : Bool := decide (Float.abs (norm_x * norm_x - dot_self) < 1e-12)
